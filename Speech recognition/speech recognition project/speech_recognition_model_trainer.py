...
class TrainModel:
    ...
    def _load_commands(self):
        commands = np.array(tf.io.gfile.listdir(str(self.dataset_path)))
    ...
    def load_datasets(self):
        ...
        self.train_ds = self.train_ds.map(self._squeeze_audio, tf.data.AUTOTUNE)
        self.val_ds = self.val_ds.map(self._squeeze_audio, tf.data.AUTOTUNE)
        self.test_ds = self.val_ds.shard(num_shards=2, index=0)
        self.val_ds = self.val_ds.shard(num_shards=2, index=1)

    @staticmethod
    def _squeeze_audio(audio, labels):
    ...
    def prepare_datasets(self):
        self.train_ds = self._make_spec_ds(self.train_ds).cache().shuffle(10000).prefetch(tf.data.AUTOTUNE)
        self.val_ds = self._make_spec_ds(self.val_ds).cache().prefetch(tf.data.AUTOTUNE)
        self.test_ds = self._make_spec_ds(self.test_ds).cache().prefetch(tf.data.AUTOTUNE)
...
class ExportModel(tf.Module):
    ...
    def __call__(self, x):
        if x.dtype == tf.string:
            ...
            x = x[tf.newaxis, :]
        x = TrainModel._get_spectrogram(x)
